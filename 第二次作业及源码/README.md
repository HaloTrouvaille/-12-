...
## 实验总结
通过本次实验，我们小组对fork、vfork创建进程方法的区别有了更深的理解和认识，也通过实验实现了进程间通过信号、管道进行通讯的方法，期间也遇到了些许问题，如signal函数与kill函数知识点模糊，但通过查找资料逐一解决。在创建管道时，通过ls可判断是否创建成功。通过这次实验，对进程的知识点与通信的知识点有了更深的认识，也意识到要更加全面的学习。
## 实验源码
见本仓库代码部分。
