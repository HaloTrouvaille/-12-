...
struct class *sculldev_class;         //创建一个class类 然后在class类下，创建//class_devices，即类下面创建类的设备
...
/*初始化模块的函数
获得内核识别的设备号，若动态分配则调用alloc_chrdev_region,若已知则调用register_chrdev_region
使用kmalloc动态的给scull_dev设备结构体在普通内核内存空间分配内存空间,并使用memset初始化为0
初始化每个设备的访问区块(即内存区块),（设定量子和量子集,互斥锁）,调用scull_setup_cdev向内核注册这个char device
